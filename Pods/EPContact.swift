...
open class EPContact {
    ...
    open func contactInitials() -> String {
        var initials = String()
        let firstNameFirstChar:String = String(firstName.prefix(1))
        if !firstNameFirstChar.isEmpty {
            initials.append(firstNameFirstChar)
        }
        let lastNameFirstChar = String(lastName.prefix(1))
        if !lastNameFirstChar.isEmpty {
